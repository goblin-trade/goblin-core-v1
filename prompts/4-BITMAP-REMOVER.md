Write a BitmapRemover with a deactivate() function that calls bitmap.clear(resting_order_index) underneath. The caching logic is similar to BitmapInserter where we use the cached bitmap group if the cached outer index is the same, to prevent expensive BitmapGroup::new_from_slot(slot_storage, outer_index) calls.
