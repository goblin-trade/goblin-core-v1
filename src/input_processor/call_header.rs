...
mod tests {
    ...
    fn test_init_with_zero_input() {
        ...
        assert_eq!(header.ix_collect_fee_count, 0);
    ...
    #[test]
    fn test_init_byte_1_collect_fee_count() {
        let mut input = [0u8; 512];
        // Set ix_collect_fee_count to 7 (maximum value for 3 bits)
        input[1] = 0b1110_0000; // 7 << 5 = 224
        
        let header = CallHeader::init(&input);
        assert_eq!(header.ix_collect_fee_count, 7);
        assert!(!header.recipient_provided);
        assert!(!header.track_msg_value);
        assert!(!header.track_eth_withdrawal_due);
        assert!(!header.deposit_shortfall);
    ...
    fn test_init_all_fields_set() {
        ...
        input[1] = 0b1111_1111; // ix_collect_fee_count=7, all flags set
    ...
    fn test_max_values_edge_cases() {
        ...
        input[1] = 0xFF; // All flags true, collect_fee_count at max (7)
