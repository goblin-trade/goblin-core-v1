use crate::{
    ...
    hostio::{self},
    input_processor::{ArgsBuffer, ArgsDecoder, HeaderFlags},
...
impl<'a> GlobalHeader<'a> {
    ...
    pub fn new(args: &'a ArgsBuffer, offset: &mut usize, len: usize) -> Result<Self, GoblinError> {
        let header = HeaderFlags::init(args, len)?;
        *offset = HeaderFlags::HEADER_BYTE_SIZE;
        ...
        let custom_erc20_list =
            args.decode_slice_unchecked::<CustomToken>(offset, header.custom_erc20_count);
