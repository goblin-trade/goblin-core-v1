...
impl From<Lots> for RawAtoms {
    /// Convert lots to raw atoms
    ///
    /// * Lots are stored in little endian format while RawAtoms are in big endian
    /// * 1 lot = 10^6 raw atoms
    /// * The conversion preserves the relationship: from_lots(to_lots(raw atoms)) == raw atoms
...
mod tests {
    ...
    mod test_atom_to_lot_conversions {
        ...
        fn test_basic_conversion() {
            ...
            let atoms = RawAtoms::from(zero_lots);
            assert_eq!(atoms.0, [0, 0, 0, 0]);

            let one_lot = Lots(1);
            let atoms = RawAtoms::from(one_lot);
            ...
            let two_lots = Lots(2);
            let atoms = RawAtoms::from(two_lots);
        ...
        fn test_roundtrip() {
            ...
            let atoms = RawAtoms::from(original_lots);
