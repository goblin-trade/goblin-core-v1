    ...
    In::Opposite: StoreReader<Tuple<Ticks, Ticks, Leg>, Result = Ticks>,
{
    ...
    while budget > taker_delta.matched_lots.taker_in {
        if let Some(resting_order_position) = resting_order_iterator.next() {
            ...
            if budget < taker_delta.matched_lots.taker_in + quote {
                // Resting order consumes the budget. Part of the resting order remains, write it back.
                let surplus = taker_delta.matched_lots.taker_in + quote - budget;
                ...
                taker_delta
                    .matched_lots
                    .checked_add(consumed, consumed_opposite)?;
            } else {
                ...
                taker_delta
                    .matched_lots
                    .checked_add(quote, quote_opposite)?;
    ...
    *In::get_leg_mut(&mut local_delta.local_sender_delta.taker_delta_pair) = taker_delta;
