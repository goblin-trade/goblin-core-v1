...
pub const HARDCODED_ERC20_STORE_HASH_LIST: [[SlotKey<StorePreimage<HardcodedERC20>>;
    HARDCODED_ERC20_LIST.inner.len()];
    HARDCODED_CALLER_LIST.inner.len()] = const {
    let mut list =
        [[SlotKey::DEFAULT; HARDCODED_ERC20_LIST.inner.len()]; HARDCODED_ERC20_LIST.inner.len()];

    let mut i = 0;
    while i < HARDCODED_ERC20_LIST.inner.len() {
