...
pub fn reduce_multiple_orders_inner(
    ...
    market.update_best_price(&bid_index_list, slot_storage);
    market.update_best_price(&ask_index_list, slot_storage);
    ...
    for (
        ...
        if last_outer_index != Some(outer_index) {
            ...
            if let Some(last_index) = last_outer_index {
                bitmap_group.write_to_slot(slot_storage, &last_index);
            }
...
pub fn insert_order_in_book(
    ...
    let inserted = insert_to_index_list(slot_storage, market_state, outer_index, side);
    ...
    'list_loop: loop {
        ...
        loop {
            ...
            loop {
                ...
                if last_stacked_outer_index
                    .is_some_and(|last_stacked_outer_index| last_stacked_outer_index == outer_index)
                {
