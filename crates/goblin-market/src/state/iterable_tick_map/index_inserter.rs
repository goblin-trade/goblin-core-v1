use super::{
    BitmapGroup, IndexListIterator, ListKey, ListSlot, OrderId, OuterIndex, SlotRestingOrder,
    TickIndices,
};
use crate::{
    program::GoblinResult,
    state::{MarketState, Side, SlotStorage},
};
use alloc::vec::Vec;
...
impl GG {
    pub fn something(&mut self, slot_storage: &mut SlotStorage) {}
}
...
pub struct IndexListInsertion<'a> {
    ...
    // Variables to optimize slot writes
    pub last_outer_index: Option<OuterIndex>,
    pub bitmap_group: BitmapGroup,
}

impl<'a> IndexListInsertion<'a> {
    pub fn new(side: Side, outer_index_count: u16, slot_storage: &'a SlotStorage) -> Self {
    ...
    pub fn gg(&self, slot_storage: &mut SlotStorage) {}
    ...
    pub fn insert_resting_order(
        ...
        // let slot_storage = self.index_list_iterator.slot_storage;
        ...
        // let needs_insertion = self.prepare(outer_index);
        ...
        // TODO last bitmap group needs to be written separately
        Ok(())
    }
...
pub fn write_prepared_indices(
    ...
    for slot_index in start_slot_index..=final_slot_index_inclusive {
        let (mut list_slot, start_relative_index) = if slot_index == start_slot_index {
            (first_list_slot, unread_count % 16)
        } else {
            (ListSlot::default(), 0)
        };
